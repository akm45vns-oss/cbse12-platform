...
export function ChapterView({ subject, chapter, curriculumData, notesRead, quizBest, onStartNotes, onStartQuiz, onStartPaper }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      <div style={{ background: "linear-gradient(135deg, #fff8fb, #f9f0f6)", border: "1.5px solid #fce7f3", borderRadius: 22, padding: 28, marginBottom: 32, boxShadow: "0 8px 24px rgba(236, 72, 153, 0.1)" }}>
        ...
        <h1 style={{ fontSize: 26, fontWeight: 900, color: "#831843", margin: "0 0 10px", letterSpacing: "-0.02em", lineHeight: 1.2 }}>{chapter}</h1>
      ...
      <div className="chapter-hub-grid">
        {[
          ...
          <button key={mode} onClick={() => {
            ...
            style={{ background: "white", border: `2px solid ${done ? color : "#fce7f3"}`, borderRadius: 20, padding: 24, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", cursor: "pointer", boxShadow: done ? `0 8px 20px ${color}20` : "0 4px 12px rgba(236, 72, 153, 0.1)" }}
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = done ? color : "#fce7f3"; e.currentTarget.style.background = "white"; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = done ? `0 8px 20px ${color}20` : "0 4px 12px rgba(236, 72, 153, 0.1)"; }}>
