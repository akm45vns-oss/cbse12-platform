  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", width: "100%", animation: "cvFadeIn 0.5s ease" }}>
      {loading ? <LoadingScreen message={loadMsg} emoji={loadEmoji} /> : quiz.length === 0 ? (
        <div style={{ textAlign: "center", padding: 60, background: "rgba(15,23,42,0.5)", backdropFilter: "blur(20px)", borderRadius: 24, border: "1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>⚠️</div>
          <div style={{ color: "#cbd5e1", fontSize: 16, marginBottom: 20 }}>{quizErr || "Failed to load quiz. Please try again."}</div>
        ...
        <div>
          <div style={{ background: "rgba(15,23,42,0.6)", backdropFilter: "blur(30px)", border: `2px solid ${score >= 40 ? "rgba(22,163,74,0.5)" : score >= 25 ? "rgba(234,179,8,0.5)" : "rgba(220,38,38,0.5)"}`, borderRadius: 24, padding: 40, textAlign: "center", marginBottom: 28, boxShadow: "0 12px 40px rgba(0,0,0,0.3)" }}>
            <div style={{ fontSize: 56, marginBottom: 16, display: "inline-block" }}>{score >= 40 ? "🏆" : score >= 25 ? "👍" : "📚"}</div>
            <div style={{ fontSize: 56, fontWeight: 900, color: "#f8fafc", letterSpacing: "-0.02em" }}>{score}<span style={{ fontSize: 26, color: "#94a3b8", fontWeight: 400, marginLeft: 4 }}>/{quiz.length}</span></div>
            <div style={{ fontSize: 22, fontWeight: 800, marginTop: 8, color: score >= 40 ? "#4ade80" : score >= 25 ? "#fde047" : "#f87171", letterSpacing: "-0.01em" }}>
              {Math.round(score / quiz.length * 100)}% Accuracy
            </div>
            <div style={{ fontSize: 16, fontWeight: 700, marginTop: 6, color: score >= 40 ? "#86efac" : score >= 25 ? "#fef08a" : "#fca5a5" }}>
          ...
          <h3 style={{ fontWeight: 900, color: "#38bdf8", marginBottom: 18, fontSize: 18, letterSpacing: "-0.01em" }}>📊 Answer Review</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {quiz.map((q, i) => {
              const ua = answers[i]; const ca = q.ans; const ok = ua === ca;
              return (
                <div key={i} style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(20px)", border: `1.5px solid ${ok ? "rgba(74,222,128,0.3)" : "rgba(248,113,113,0.3)"}`, borderRadius: 16, padding: 18 }}>
                  <div style={{ display: "flex", gap: 12, marginBottom: ok ? 8 : 10 }}>
                    <span style={{ background: ok ? "rgba(22,163,74,0.3)" : "rgba(220,38,38,0.3)", color: ok ? "#86efac" : "#fca5a5", fontSize: 12, fontWeight: 800, padding: "4px 10px", borderRadius: 8, flexShrink: 0, alignSelf: "flex-start", border: `1px solid ${ok ? "#16a34a" : "#dc2626"}` }}>Q{i + 1} {ok ? "✓" : "✗"}</span>
                    <span style={{ fontSize: 14, color: "#f8fafc", lineHeight: 1.6, fontWeight: 600 }}>{q.q}</span>
                  </div>
                  {!ok && (
                    <div style={{ marginLeft: 60, fontSize: 13, marginBottom: 8, background: "rgba(0,0,0,0.2)", padding: 10, borderRadius: 8 }}>
                      <div style={{ color: "#fca5a5", fontWeight: 600 }}>Your answer: {q.opts[ua] || "Not answered"}</div>
                      <div style={{ color: "#86efac", fontWeight: 700, marginTop: 4 }}>Correct: {q.opts[ca]}</div>
                    </div>
                  )}
                  <div style={{ marginLeft: 60, marginTop: 10, fontSize: 13, color: "#a5b4fc", fontWeight: 500, lineHeight: 1.6 }}>💡 {q.exp}</div>
                </div>
              );
            })}
          </div>
        ...
        <div>
          <div className="no-print" style={{ background: "rgba(30,41,59,0.5)", borderRadius: 18, border: "1.5px solid rgba(255,255,255,0.1)", padding: "16px 22px", marginBottom: 20, backdropFilter: "blur(20px)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div>
                <span style={{ fontSize: 14, fontWeight: 800, color: "#38bdf8", letterSpacing: "-0.01em" }}>Question {qIdx + 1} of {quiz.length}</span>
                <span style={{ fontSize: 12, color: "#cbd5e1", marginLeft: 14, fontWeight: 600 }}>{Object.keys(answers).length} answered</span>
              </div>
          ...
          {quiz[qIdx] && (
            <div style={{ background: "rgba(15,23,42,0.6)", borderRadius: 20, border: "1.5px solid rgba(255,255,255,0.08)", padding: "28px 28px", marginBottom: 18, backdropFilter: "blur(20px)" }}>
              ...
              <p style={{ fontSize: 16, fontWeight: 700, color: "#f8fafc", lineHeight: 1.7, marginBottom: 24, letterSpacing: "-0.01em" }}>{quiz[qIdx].q}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {validateQuestion(quiz[qIdx])?.opts?.map((opt, oi) => {
                  ...
                  return (
                    <button key={oi} onClick={() => setAnswers(a => ({ ...a, [qIdx]: oi }))}
                      style={{
                        padding: "16px", borderRadius: 14, border: `1.5px solid ${isSelected ? (S?.accent || "#6366f1") : "rgba(255,255,255,0.1)"}`,
                        background: isSelected ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.03)", color: isSelected ? "#f8fafc" : "#cbd5e1",
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
            <button onClick={() => setQIdx(q => Math.max(0, q - 1))} disabled={qIdx === 0}
              style={{ padding: "12px 24px", background: "rgba(255,255,255,0.05)", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#cbd5e1", fontWeight: 700, fontSize: 15, opacity: qIdx === 0 ? 0.4 : 1, cursor: qIdx === 0 ? "not-allowed" : "pointer" }}>
              ← Prev
            </button>
            {qIdx < quiz.length - 1 ? (
          ...
          <div style={{ background: "rgba(15,23,42,0.4)", borderRadius: 18, border: "1.5px solid rgba(255,255,255,0.08)", padding: "18px 20px", backdropFilter: "blur(20px)" }}>
              ...
              {quiz.map((_, i) => (
                <button key={i} onClick={() => setQIdx(i)}
                  style={{
                    ...
                    background: i === qIdx ? (S?.accent || "#6366f1") : answers[i] !== undefined ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.05)",
                    color: i === qIdx ? "white" : answers[i] !== undefined ? "#4ade80" : "#94a3b8",
